...
const DashBoard = (props) => {
    ...
    const [token, setToken] = React.useState('u0sfyxkfue-aIg68Jj+m+wMHM/+DDWb0KZBDFlo7NRaiC65vyInDn8=');
    ...
    const kaleidoStatus = () => { 
        const url = 'https://console-us1.kaleido.io/api/v1/consortia/u1q22yi6qo/environments/u1twdviblf/status';
        let config = { "headers" : {
    ...
    const  start = () => {  
        ...
        let body ={
            ...
            "variables": {"environment": {"state": "live"}, "consortia_id": "u1q22yi6qo", "id": "u1twdviblf"},
    ...
    const  stop = () => {  
        ...
        let body ={
            "operationName": "updateEnvironment",
            "variables": {"environment": {"state": "paused"}, "consortia_id": "u1q22yi6qo", "id": "u1twdviblf"},
            "query": "fragment EnvironmentFields on Environment {\n  _id\n  name\n  description\n  state\n  mode\n  provider\n  release_id\n  consensus_type\n  consortia_id\n  chain_id\n  block_period\n  region\n  enable_tether\n  limit_initial_signers\n  node_list\n  service_list\n  autopause_init_delay\n  autopause_idle_hours\n  created_at\n  deleted_at\n  paused_at\n  resumed_at\n  member_policy\n  amend_policy\n  locked_policies\n  test_features\n  updated_at\n  __typename\n}\n\nmutation updateEnvironment($consortia_id: String!, $id: String!, $environment: UpdateEnvironmentInput!) {\n  updateEnvironment(consortia_id: $consortia_id, id: $id, environment: $environment) {\n    ...EnvironmentFields\n    __typename\n  }\n}\n"
        }    
        let config = { "headers" : {
                        "Authorization" : 'Bearer ' + token, 
    ...
    const stopNode = () => {
        ...
        console.log('mat1-stop day: ' + day + '  Hours : ' + hours + '</br>' + min + seg);
        if (hours === 20 && min === 57) 
        {
    ...
    const startNode = () => {
        ...
        console.log('mat1-start day: ' + day + '  Hours : ' + hours + '</br>' + min + seg);
        //8 am a 7 pm
        if (hours === 8 && min === 3) 
        {
                                        ...
                                        <Grid container spacing={3}>
                                            ...
                                            <Grid item>Api nodo 3: <b>{node3}</b></Grid>*/}
                                              <Tooltip title="Cambiar estatus Environment ">
                                                        {nodes.state==='paused' ?
                                                    <Button  onClick={() => start()}> Materialidad Uno / Environment  :  start
                                                        <LoopIcon/>
                                                    </Button> :  
                                                    <Button  onClick={() => stop()}>  Materialidad Uno / Environment  :  stop
                                                        <LoopIcon/>
                                                    </Button>
                                            }</Tooltip>
